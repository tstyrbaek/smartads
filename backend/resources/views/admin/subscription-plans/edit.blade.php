        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.subscription-plans.update', $plan) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PATCH')

                        <div>
                            <label class="block font-medium text-sm text-gray-700" for="name">Navn *</label>
                            <input id="name" name="name" type="text" value="{{ old('name', $plan->name) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $plan->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input id="max_tokens_per_month" name="max_tokens_per_month" type="number" min="1" value="{{ old('max_tokens_per_month', $plan->max_tokens_per_month) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                            ...
                            <div>
                                ...
                                <input id="price_per_month" name="price_per_month" type="number" step="0.01" min="0" value="{{ old('price_per_month', $plan->price_per_month) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
                        ...
                        <div class="flex items-center gap-3">
                            <input id="is_active" name="is_active" type="checkbox" value="1" {{ $plan->is_active ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" />
                            ...
                            <div id="features-container" class="mt-2 space-y-2">
                                @foreach(old('features', $plan->features ?? []) as $feature)
                                    <div class="feature-input-group">
                                        <input type="text" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" name="features[]" placeholder="Indtast feature" value="{{ $feature }}" required>
                                ...
                                @endforeach
                                @if(count(old('features', $plan->features ?? [])) === 0)
                                    <div class="feature-input-group">
                                        <input type="text" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" name="features[]" placeholder="Indtast feature" required>
                                        <button type="button" class="remove-feature inline-flex items-center px-3 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">Slet</button>
                                    </div>
                                @endif
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addFeatureInput(value = '') {
        ...
        div.innerHTML = `
            <input type="text" class="block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" name="features[]" placeholder="Indtast feature" value="${value}">
            <button type="button" class="remove-feature inline-flex items-center px-3 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">Slet</button>
        `;
