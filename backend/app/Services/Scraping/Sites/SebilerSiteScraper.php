...
class SebilerSiteScraper implements SiteScraperInterface
{
    ...
    public function scrape(string $url): array
    {
        ...
        if (preg_match('/bid-(\d+)/i', $url, $idMatch) !== 1) {
            throw new \RuntimeException('sebiler_vehicle_id_missing');
        }
        $vehicleId = $idMatch[1];

        if (preg_match('~<script[^>]+id="usedcarsmodule"[^>]+src="([^"]+)"~i', $html, $scriptMatch) !== 1) {
            throw new \RuntimeException('sebiler_module_script_missing');
        }
        ...
        if (preg_match('/listId:\s*(\d+)/', $moduleScript, $listIdMatch) !== 1) {
            throw new \RuntimeException('sebiler_list_id_missing');
        }
        ...
        if (!is_array($data)) {
            throw new \RuntimeException('sebiler_api_invalid');
        }
        ...
        $specifications = [
            ...
            'Økonomi' => array_filter([
                'Grøn ejerafgift' => trim((string) ($car['GreenTax'] ?? '')) !== '' ? trim((string) $car['GreenTax']) . ' Kr. / årligt' : '',
            ], static fn ($value): bool => is_string($value) && $value !== ''),
        ];
