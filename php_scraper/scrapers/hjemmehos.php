...
final class HjemmehosSiteScraper implements SiteScraperInterface
{
    ...
    public function scrape(string $url): array
    {
        ...
        $findSectionContentByHeading = static function (DOMXPath $xpath, string $heading): ?DOMElement {
            $query = sprintf(
                ...
                HjemmehosScraperHelper::xpathLiteral($heading)
            );
        ...
        if ($title === '' && isset($ldJson['name']) && is_string($ldJson['name'])) {
            $title = trim($ldJson['name']);
        }
        if ($title === '' && isset($productData['product_title']) && $productData['product_title'] !== '') {
        ...
        if ($brand === '' && isset($ldJson['brand']) && is_string($ldJson['brand'])) {
        ...
        if ($description === null && isset($ldJson['description']) && is_string($ldJson['description'])) {
        ...
        $images = [];
        if (isset($ldJson['image']['url']) && is_string($ldJson['image']['url'])) {
            $images[] = $resolveUrl($ldJson['image']['url'], $url);
        }
        if (isset($productData['img_url']) && $productData['img_url'] !== '') {
        ...
        $offers = [];
        if (isset($ldJson['offers']) && is_array($ldJson['offers'])) {
        ...
        $realtorUrl = isset($ldJson['url']) && is_string($ldJson['url']) ? $ldJson['url'] : $url;
        ...
        return [
            ...
            'product_id' => $productData['product_id'] !== '' ? $productData['product_id'] : null,
            'current_variant_id' => $productData['variant_id'] !== '' ? $productData['variant_id'] : null,
            'product_code' => $productData['product_code'] !== '' ? $productData['product_code'] : null,
            'handle' => $productData['handle'] !== '' ? $productData['handle'] : null,
        ];
    }
}

final class HjemmehosScraperHelper
{
