...
final class BoesenbaekSiteScraper implements SiteScraperInterface
{
    ...
    public function scrape(string $url): array
    {
        ...
        $extractFieldByLabel = static function (DOMXPath $xpath, string $label): ?string {
            $query = sprintf(
                '//p[contains(@class, "bdt_spec_label")][normalize-space(.)=%s]/following-sibling::p[contains(@class, "bdt_spec_value")][1]',
                BoesenbaekScraperHelper::xpathLiteral($label)
            );
...
final class BoesenbaekScraperHelper
{
